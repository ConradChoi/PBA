...
/admin/consulting-requests      list: requested_at, preferred_contact, message,
                                 linked assessment's name/level — each row links to
                                 the existing public /diagnose/result/[assessmentId]
/admin/assessments              list: name, email, business_stage, architecture_level,
                                 total_raw, created_at — each row links to the same
                                 result page
/admin/operators                owner-only. List: email, role, created_at.
                                 Add form: email + temporary password + role select.
                                 Delete button per row.
```

No new assessment/consulting-request detail view is built — both lists link out to the already-public `/diagnose/result/[assessmentId]` page, which already shows everything about a given diagnosis except the consulting request's own fields (shown inline in the consulting-requests list instead).
...
Unit-tested (Vitest, existing pattern): the consulting-request schema (required `preferredContact`, optional `message`), the `POST /api/assessments/[assessmentId]/consulting-requests` route (mocked Supabase client — insert + `consulting_requested` update, 404 for a missing assessment), and the two operator-management routes (mocked `supabase.auth.admin.*` calls — role rejection for `staff`, the two delete guards, success paths).

Not automated: the login flow, the admin list pages, and the operator-management UI — consistent with the project's existing choice not to add component/E2E testing infrastructure. Verified manually: log in as the bootstrapped owner, view both lists, add a `staff` operator, confirm that account can log in and see the lists but not `/admin/operators`, confirm `staff` gets 403 from the operator API routes directly, then delete the test `staff` account as `owner`.
